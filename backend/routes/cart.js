  ...
  try {
    ...
    if (existing.rows[0]) {
      ...
      result = await pool.query(
        "UPDATE cart SET quantity = quantity + $1 WHERE cart_id = $2 RETURNING cart_id, quantity",
        [qty, existing.rows[0].cart_id]
      );
    } else {
      result = await pool.query(
        "INSERT INTO cart (user_id, product_id, quantity, added_at) VALUES ($1, $2, $3, NOW()) RETURNING cart_id, quantity",
